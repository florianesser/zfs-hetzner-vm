...
Ubuntu 20 LTS support (c) 2021 wetransform GmbH
...
function ask_free_tail_space {
  ...
  while [[ ! $v_free_tail_space =~ ^[0-9]+$ ]]; do
    v_free_tail_space=$(dialog --ascii-lines --inputbox "${tail_space_invalid_message}Enter the space to leave at the end of each disk (in MiB, 0 for none):" 30 100 "$c_default_free_tail_space" 3>&1 1>&2 2>&3)
...
function ask_zfs_arc_max_size {
  ...
  while [[ ! $v_zfs_arc_max_mb =~ ^[0-9]+$ ]]; do
    v_zfs_arc_max_mb=$(dialog --ascii-lines --inputbox "${zfs_arc_max_invalid_message}Enter ZFS ARC cache max size in MiB (minimum 64 MiB, enter 0 for ZFS default value, the default will take up to 50% of memory):" 30 100 "$c_default_zfs_arc_max_mb" 3>&1 1>&2 2>&3)
...
function ask_hostname {
  ...
  while [[ ! $v_hostname =~ ^[a-z][0-9a-zA-Z_-]+$ ]]; do
...
echo "======= installing zfs on rescue system =========="
  ...
  echo "deb https://terem42.github.io/zfs-debian/public zfs-debian-experimental main" > /etc/apt/sources.list.d/zfs-debian.list
  apt update
  apt install --yes -t zfs-debian-experimental zfs-dkms zfsutils-linux
  modprobe zfs
  zfs --version
...
echo "======= partitioning the disk =========="
  ...
  if [[ $v_free_tail_space -eq 0 ]]; then
    ...
    tail_space_parameter="-${v_free_tail_space}M"
  fi
...
zfs create -o canmount=off -o mountpoint=none "$v_rpool_name/USERDATA"
...
zfs create -o mountpoint=/root             "$v_rpool_name/USERDATA/root"
zfs create -o canmount=off                 "$v_rpool_name/ROOT/ubuntu/var"
zfs create -o canmount=off                 "$v_rpool_name/ROOT/ubuntu/var/lib"
zfs create                                 "$v_rpool_name/ROOT/ubuntu/var/log"
zfs create                                 "$v_rpool_name/ROOT/ubuntu/var/spool"

zfs create -o com.sun:auto-snapshot=false  "$v_rpool_name/ROOT/ubuntu/var/cache"
zfs create -o com.sun:auto-snapshot=false  "$v_rpool_name/ROOT/ubuntu/var/tmp"
chmod 1777 "$c_zfs_mount_dir/var/tmp"

zfs create                                 "$v_rpool_name/ROOT/ubuntu/srv"

zfs create -o canmount=off                 "$v_rpool_name/ROOT/ubuntu/usr"
zfs create                                 "$v_rpool_name/ROOT/ubuntu/usr/local"

zfs create                                 "$v_rpool_name/ROOT/ubuntu/var/mail"

zfs create -o com.sun:auto-snapshot=false -o canmount=on -o mountpoint=/tmp "$v_rpool_name/ROOT/ubuntu/tmp"
...
apt install -t buster-backports debootstrap
...
deb [arch=i386,amd64] $c_deb_packages_repo focal main restricted universe multiverse
deb [arch=i386,amd64] $c_deb_packages_repo focal-updates main restricted universe multiverse
deb [arch=i386,amd64] $c_deb_packages_repo focal-backports main restricted universe multiverse
deb [arch=i386,amd64] $c_deb_security_repo focal-security main restricted universe multiverse
CONF
...
chroot_execute "apt install --yes man-db wget curl software-properties-common joe htop tmux"
chroot_execute "systemctl disable thermald"

echo "======= installing zfs packages =========="
chroot_execute 'echo "zfs-dkms zfs-dkms/note-incompatible-licenses note true" | debconf-set-selections'
chroot_execute "apt install --yes zfs-initramfs zfsutils-linux"
...
chroot_execute "zfs set mountpoint=legacy $v_rpool_name/ROOT/ubuntu/var/log"
chroot_execute "echo $v_rpool_name/var/log /var/log zfs nodev,relatime 0 0 >> /etc/fstab"
chroot_execute "zfs set mountpoint=legacy $v_rpool_name/ROOT/ubuntu/var/spool"
chroot_execute "echo $v_rpool_name/var/spool /var/spool zfs nodev,relatime 0 0 >> /etc/fstab"
chroot_execute "zfs set mountpoint=legacy $v_rpool_name/ROOT/ubuntu/var/tmp"
chroot_execute "echo $v_rpool_name/var/tmp /var/tmp zfs nodev,relatime 0 0 >> /etc/fstab"
chroot_execute "zfs set mountpoint=legacy $v_rpool_name/ROOT/ubuntu/tmp"
